...
def build_status(targets: set[str] | None = None) -> dict:
  selected = set(TARGETS if targets is None else targets)
  sdk_selected = set(SDK_REQUIRED_STATUS if targets is None else targets)
  return {
    "generated": {
      key: details
      for target, (key, details) in GENERATED_STATUS.items()
      if target in selected
    },
    "sdkRequired": {
      target: details
      for target, details in SDK_REQUIRED_STATUS.items()
      if target in sdk_selected
    },
  }
...
def build(output: Path, targets: set[str] | None = None) -> None:
  ...
  (output / "homebrew-status.json").write_text(
    json.dumps(build_status(targets), indent=2) + "\n",
    encoding="utf-8",
  )
